// Code generated by jk generate transport -t Service -P http -s -l go -f gin --swagger --embed-swagger; DO NOT EDIT.
...
var ginEmbedSwagger embed.FS

// RegisterEmbedSwaggerUI register embed swagger-ui urls
func (s *GinServerSet) RegisterEmbedSwaggerUI(r gin.IRouter) {
	fs := http.FS(ginEmbedSwagger)
	handler := http.FileServer(fs)
	handler = http.StripPrefix("/swagger/service/spec/", handler)
	r.GET(
		"/swagger/service/spec/*rest",
		gin.WrapH(handler))
	u := httpSwagger.URL("/swagger/service/spec/swagger.json")
	handler = httpSwagger.Handler(u)
	r.GET("/swagger/service/swagger-ui/*rest", gin.WrapH(handler))
}

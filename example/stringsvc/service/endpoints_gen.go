...
func (s ServiceEndpoints) Join3(ctx context.Context, parts *map[string]string) (text string, err error) {
	request := &Join3Request{Parts: parts}
	resp, err := s.join3(ctx, request)
	if err != nil {
		return "", err
	}
	response := resp.(*Join3Response)
	text = response.Text
	return text, nil
}
